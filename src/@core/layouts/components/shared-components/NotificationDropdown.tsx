    ...
    <Fragment>
                ...
                <Box sx={{ mr: 4, ml: 2.5, flex: '1 1', display: 'flex', overflow: 'hidden', flexDirection: 'column' }} onClick={() => handleDetailNoti(notification)}>
                  <MenuItemTitle>
                    {notification.meta_data.sender.role === 'Admin' ?
                      (<strong>System</strong>) : <strong>{`${notification.meta_data.sender.lastName} ${notification.meta_data.sender.firstName}`}</strong>
                    }
                  </MenuItemTitle>
      ...
      <Dialog maxWidth={'md'} open={openNotiDetail} onClose={handleCloseNotiDetail} aria-labelledby='full-screen-dialog-title' sx={{
        ...
        <DialogContent sx={{ margin: '0px auto' }}>
          <DialogTitle id='full-screen-dialog-title' sx={{ 'text-align': 'center' }}>
            {noti?.title}
          </DialogTitle>
          <Box sx={{marginTop: '15px', display: 'flex', justifyContent: 'center'}}>
            <Button onClick={handleCloseNotiDetail}>Close</Button>
            <Button onClick={() => handlePostDetails(noti?.meta_data?.postId, noti?.is_read, noti?._id)}>Go to Pose</Button>
          </Box>
        </DialogContent>
      </Dialog>
    </Fragment>
